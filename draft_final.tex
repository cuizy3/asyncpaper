...
We first take note of some properites of semi-modular and fault-tolerant circuits.  \\
...
\begin{lemma}[4]
For any faulty trace $\sigma$ in the fault-tolerant circuit where the A-half stops, if the transient fault occurs on wires $c1^B$ or $v^B$ of some wireblock $v$, then the circuit stops and for each wireblock $w$, wire values $w^A=w^B$.
\end{lemma}

\begin{proof}
The fault is in the B-half while the A-half is fault-free so Lemma 1 applies for the A-half.  We will show by induction on $n$ that Lemma 4 is true for each prefix of the trace, $\sigma_n$.  %how to make it seem like prefix not trace modifies sigma_n
...
Therefore we do not consider $c1^B$ transitions because the transition is covered by X. %X covers both cases when $c1^B$ is 0 or 1.
...
\begin{itemize}
	\item
Case 1:  If the wireblock is in case 1, its $c2$ and $c3$ wires are not excited.  Depending on the inputs to the wireblock, wire $c1^A$ may be excited.  Therefore the wireblock either stays in this case or transitions to case 2 in state $s_{k+1}$.  %(must prove both the state, and the transition, if there is any... follows the state graph)
\item
