...
\begin{definition} A {\em valid initial state} for the A half of the fault tolerant circuit is constructed from the initial state of the original circuit.  The wire values in the logic block and $w$ in the original circuit are copied to the corresponding wires in the A half.  The additional wires from the C-element chain are also assigned the same value as $w$.  That is, first assign the value of $w$ to $w^A$, then assign the value of $w$ to $c1^A$ $c2^A$ $c3^A$ as well. The same construction applies for the B half circuit as well.
\end{definition}
%\textbf{
...
%since it's a transition, then all previous transitions must also be in the set of traces, in fact they must all be a sequence of at least one trace (true because if excited, must be excited in all states of same name)
 original circuit.  This is a strong result; it is true for arbitrary behavior of
