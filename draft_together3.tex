...
We first define a trace, to describe the different paths a circuit can take as it follows the state graph.  Different paths can occur because each state may have multiple transitions from that state.  We record the succession of states and transitions by the trace which is defined as follows:
